  ...
  <div :class="{'open':isSort || isScreen}" @click.self="hideView()">
    ...
    <section class="filter-extend" v-if="isScreen">
      ...
      <div class="morefilter-btn">
        ...
        <span class="morefilter-ok" @click="filterOk()">确认</span>
      </div>
    </section>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    choose(item, screen) {
      if (screen.id !== "MPI" && item.select == false) {
        screen.data.forEach(ele => {
          ele.select = false;
        }),
          (item.select = !item.select);
      } else if (screen.id !== "MPI" && item.select == true) {
        item.select = false;
      } else {
        item.select = !item.select;
      }
